...
const pendingChecklists = async () => {
    ...
    for (const iterator of pendingRecordsEquipemtTags) {
        ...
        cobieTags.push({
            ...
            date: `${iterator.day_of_month}-${new Date().getMonth() + 1}-${new Date().getFullYear()}`
        });
    }
    ...
    const filteredPendingTags = cobieTags.filter((item) => !submittedCobieTags.includes(item.cobie_tag));
